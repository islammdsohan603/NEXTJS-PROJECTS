          ...
          data.map(friend => (
            ...
            >
              ...
              <div className="flex items-center justify-center">
                <span
                  className={` px-4 py-1 rounded-full ${friend.status === 'overdue' ? 'bg-red-500' : friend.status === 'almost due' ? 'bg-yellow-400' : friend.status === 'on-track' ? 'bg-green-800 text-white' : ''}`}
                >
                  {' '}
                  {friend.status}{' '}
                </span>
